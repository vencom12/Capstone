...
exports.updateInventoryItem = async (req, res) => {
    try {
        const { count, minThreshold, item, unit } = req.body;
...
exports.updateGlobalThreshold = async (req, res) => {
    try {
        ...
        await Inventory.updateMany({}, { $set: { minThreshold: parseInt(minThreshold) } });
...
exports.updateOrdersStatus = async (req, res) => {
    try {
        ...
        // Sync linked transactions
        const txIds = orders.map(o => o.transactionId).filter(Boolean);
        if (txIds.length > 0) {
            let txStatus = 'processing';
            if (status === 'Completed' || status === 'Order Delivered') txStatus = 'completed';
            if (status === 'Order Canceled') txStatus = 'canceled';
            await Transaction.updateMany({ _id: { $in: txIds } }, { $set: { status: txStatus } });
        }
...
exports.getSettings = async (req, res) => {
    // For MongoDB, we could use a separate collection, but for now just return default
    res.json({ id: 'global', inventoryAuditLog: true });
};

exports.updateSettings = async (req, res) => {
    res.json({ message: 'Settings update not implemented for MongoDB yet', ...req.body });
};
