...
const _updateUIInternal = debounce(() => {
    ...
    const inventorySearch = document.getElementById('admin-inventory-search')?.value.toLowerCase();
    if (inventorySearch) {
    ...
    if (analytics) {
        ...
        // Update Analytics Tab Cards
        const visitsEl = document.getElementById('analytics-total-visits');
        if (visitsEl) visitsEl.innerText = analytics.totalVisits || 0;

        const avgEl = document.getElementById('analytics-avg-order');
        if (avgEl) avgEl.innerText = `$${parseFloat(analytics.avgOrderValue || 0).toFixed(2)}`;

        const peakEl = document.getElementById('analytics-peak-season');
        if (peakEl) peakEl.innerText = analytics.peakSeason || '---';

        // Low stock indicators (midnight/gold) - Updated to use .item instead of .name
        const midnight = inventory.find(i => (i.item || "").toLowerCase().includes('midnight'))?.count || 0;
        const gold = inventory.find(i => (i.item || "").toLowerCase().includes('gold'))?.count || 0;
                ...
                <td data-label="Alert Threshold" style="color: var(--text-dim);">${item.minThreshold || 10}</td>
                        ...
                        <td data-label="Action">
                            <div style="display: flex; gap: 8px;">
                                ...
                                <button class="btn" onclick="deleteInventoryItem('${item.id}', '${item.item}')" style="background: rgba(239, 68, 68, 0.1); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); padding: 6px 12px; font-size: 0.8rem;">Delete</button>
                            </div>
                        </td>
            </tr>
        `;
...
async function deleteInventoryItem(id, name) {
    ...
    try {
        ...
        showToast('Material removed from inventory', 'success');
...
async function openUpdateStockModal(id, name, currentCount) {
    await ModalManager.open('UPDATE_STOCK', {
        onOpen: (modal) => {
            ...
            // Ensure threshold field is visible for admin
            ...
            // Reset button state just in case it was stuck
            if (submitBtn) {
...
async function updateGlobalThreshold() {
    ...
    try {
        ...
        if (window.showToast) showToast('Global threshold applied successfully!');
        if (window.refreshDashboardState) refreshDashboardState(); // Fetch new data and refresh UI

        document.getElementById('global-threshold-input').value = '';
    } catch (err) {
        console.error(err);
        if (window.showToast) showToast('Failed to update global threshold', 'error');
    }
}

async function fetchInventoryLogs() {
    try {
        const res = await fetch(`${API_URL}/inventory/logs`, { credentials: 'include' });
        if (!res.ok) return;
            ...
            <div style="display: flex; gap: 12px; align-items: flex-start; padding: 10px; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px solid var(--border-glass);">
                ...
                <div style="flex: 1;">
                    <p style="margin: 0; font-size: 0.9rem; color: var(--text-main);">
                        <span style="font-weight: 600;">${log.user?.username || log.userId}</span> 
                        ${log.action.toLowerCase()}ed 
                        <span style="font-weight: 600; color: ${iconColor};">${log.amount}</span> 
                        of <span style="font-weight: 600;">${log.inventory?.item || 'Unknown Item'}</span>
                    </p>
...
async function refreshDashboardState() {
    ...
    try {
        ...
        if (data) {
            ...
            fetchInventoryLogs(); // Refresh logs as part of the cycle
        }
    } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    // Search listeners
    ['admin-orders-search', 'admin-products-search', 'admin-staff-search', 'admin-history-search', 'admin-inventory-search'].forEach(id => {
    ...
    // Create Product Form
    // Create Product Form - Simplified to use global listener
    const productForm = document.getElementById('create-product-form');
    if (productForm) {
        productForm.onsubmit = (e) => e.preventDefault();
    }
...
window.openCreateProduct = async () => {
    ...
    if (form) {
        ...
        // Reset recipe UI
        document.getElementById('recipe-list').innerHTML = '<p id="recipe-empty-msg" style="margin: 0; color: var(--text-dim); font-size: 0.85rem; font-style: italic;">No materials added yet.</p>';
...
function populateMaterialSelect() {
    ...
    select.innerHTML = '<option value="">Select Material...</option>' +
        inventory.map(i => `<option value="${i.id}">${i.item} (${i.count} ${i.unit || 'units'} left)</option>`).join('');
}

window.addRecipeComponent = () => {
    const select = document.getElementById('recipe-material-select');
    const qtyInput = document.getElementById('recipe-quantity-input');
    ...
    if (!id || !qty || qty <= 0) return showToast('Please select a material and enter quantity');

    if (emptyMsg) emptyMsg.remove();

    // Check if already added
    if (list.querySelector(`[data-id="${id}"]`)) return showToast('Material already in recipe');
    ...
    item.style = 'display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px; border: 1px solid var(--border-glass);';
    item.innerHTML = `
        <span style="font-size: 0.9rem; color: white;">${name} <b style="color: var(--primary); margin-left: 5px;">x${qty}</b></span>
        <span style="cursor: pointer; color: #ef4444; font-weight: 700;" onclick="this.parentElement.remove(); checkRecipeEmpty();">✕</span>
    `;
...
window.checkRecipeEmpty = () => {
    const list = document.getElementById('recipe-list');
    if (list.children.length === 0) {
        list.innerHTML = '<p id="recipe-empty-msg" style="margin: 0; color: var(--text-dim); font-size: 0.85rem; font-style: italic;">No materials added yet.</p>';
    }
};
...
window.openEditProduct = async (id) => {
    ...
    if (form) {
        ...
        if (product.recipe && product.recipe.length > 0) {
            product.recipe.forEach(comp => {
                const item = document.createElement('div');
                item.className = 'recipe-item';
                item.dataset.id = comp.inventoryId;
                item.dataset.name = comp.name;
                item.dataset.qty = comp.quantity;
                item.style = 'display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.05); padding: 8px 12px; border-radius: 6px; border: 1px solid var(--border-glass);';
                item.innerHTML = `
                    <span style="font-size: 0.9rem; color: white;">${comp.name} <b style="color: var(--primary); margin-left: 5px;">x${comp.quantity}</b></span>
                    <span style="cursor: pointer; color: #ef4444; font-weight: 700;" onclick="this.parentElement.remove(); checkRecipeEmpty();">✕</span>
                `;
                list.appendChild(item);
            });
...
document.addEventListener('submit', async (e) => {
    ...
    if (e.target.id === 'create-product-form') {
        ...
        const recipeItems = Array.from(document.querySelectorAll('.recipe-item')).map(item => ({
            inventoryId: item.dataset.id,
            name: item.dataset.name,
            quantity: parseFloat(item.dataset.qty)
        }));
        formData.append('recipe', JSON.stringify(recipeItems));
...
window.handleAuditLogToggle = async (checkbox) => {
    try {
        const enabled = checkbox.checked;
        const res = await apiFetch(`${API_URL}/settings`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ inventoryAuditLog: enabled })
        });
        if (res.ok) {
            showToast(`Audit Trail ${enabled ? 'Enabled' : 'Disabled'}`, 'success');
        } else {
            throw new Error('Failed to save settings');
        }
    } catch (err) {
        console.error(err);
        showToast('Failed to update audit settings', 'error');
        checkbox.checked = !checkbox.checked; // Revert UI
    }
};

// Hook into ModalManager to load settings when opening the modal
const originalOpenModal = ModalManager.open;
ModalManager.open = async function (key, options) {
    const result = await originalOpenModal.apply(this, arguments);
    if (key === 'INVENTORY_SETTINGS') {
        try {
            ...
            if (res.ok) {
                ...
                if (toggle) toggle.checked = settings.inventoryAuditLog;
            }
