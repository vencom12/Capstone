...
export default function CheckoutModal({ isOpen, onClose }: CheckoutModalProps) {
  ...
  return (
    <GlassModal isOpen={isOpen} onClose={onClose} maxWidth="max-w-[750px]">
      <div className="p-6">
        ...
        <div className="grid grid-cols-[1.1fr_1fr] gap-8 max-[800px]:grid-cols-1">
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-1.5">
                  <input 
                    ...
                    className="w-full bg-white/5 border border-border-glass p-2.5 rounded-xl text-text-dim text-[0.9rem] cursor-not-allowed"
                  />
                  <p className="text-[0.7rem] text-text-dim ml-1">To change address, please update your profile settings.</p>
                </div>
                <input 
                  type="text" 
                  ...
                  className="w-full bg-bg-surface border border-border-glass p-2 rounded-xl text-text-main text-[0.85rem] outline-none focus:border-primary"
                />
                <textarea 
                  ...
                  className="w-full bg-bg-surface border border-border-glass p-2 rounded-xl text-text-main text-[0.85rem] outline-none focus:border-primary resize-none"
                />
              </div>
            </div>

            <div className="flex flex-col gap-3 border-t border-border-glass pt-5">
          ...
          {/* Right: Payment */}
          <div className="flex flex-col gap-6 bg-white/5 border border-border-glass p-6 rounded-[24px]">
            <div className="flex flex-col gap-4">
              <h3 className="text-[1rem] font-bold m-0">Payment Method</h3>
              
              {/* Wallet Card */}
              <div className="bg-gradient-to-br from-primary/20 to-secondary/20 border border-primary/30 p-3 rounded-xl flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className="text-[0.75rem] text-text-dim">Wallet Balance</span>
                  <span className="text-lg font-extrabold text-white font-mono">${user?.walletBalance?.toFixed(2) || '0.00'}</span>
                </div>
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 bg-black/30 border border-white/10 p-2 rounded-lg text-white text-[0.85rem] outline-none focus:border-primary"
                  />
                  <button 
                    onClick={handleTopUp}
                    className="bg-primary text-white px-3 py-2 rounded-lg text-[0.8rem] font-bold hover:bg-primary/80 transition-all"
                  >
                    Top Up
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    {m.icon}
                    <span className="text-[0.7rem] font-bold uppercase tracking-tight">{m.label}</span>
                  </button>
                ))}
              </div>
              ...
              {(paymentMethod === 'gcash' || paymentMethod === 'paymaya') && (
                <div className="flex flex-col gap-3 border-t border-border-glass pt-4 mt-2 animate-[fadeIn_0.3s_ease-out]">
                  <h4 className="text-[0.85rem] font-bold m-0 flex items-center gap-2">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" /></svg>
                    Upload Payment Receipt
                  </h4>
                  <div 
                    onClick={() => document.getElementById('receipt-upload')?.click()}
                    className="border-2 border-dashed border-border-glass rounded-xl p-5 text-center cursor-pointer hover:border-primary/50 transition-all bg-black/20"
                  >
                    <input id="receipt-upload" type="file" className="hidden" accept="image/*" onChange={handleFileSelect} />
                    {receiptFile ? (
                      <span className="text-[0.85rem] text-primary font-medium">{receiptFile.name}</span>
                    ) : (
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <span className="text-[0.75rem] text-text-dim">Click to upload screenshot</span>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between border-t border-border-glass pt-4 mt-auto">
                <span className="text-[0.85rem] text-text-dim">Payment Status:</span>
                <span className={`
              ...
              <div className="mt-4">
                <GlassButton
                  ...
                  className={!paymentVerified ? 'opacity-50 cursor-not-allowed' : ''}
                >
