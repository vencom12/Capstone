    ...
    <header className="sticky top-0 w-full shrink-0 h-[var(--header-height)] z-[2000] flex items-center justify-between px-10 bg-bg-header backdrop-blur-[15px] border-b border-border-glass transition-all duration-300 
      ...
      <div className="flex items-center gap-3 justify-end max-[1250px]:order-2">
        ...
        <div className="relative cursor-pointer hover:opacity-80 shrink-0" onClick={() => useUIStore.getState().setBasketOpen(true)}>
          ...
          {basketCount > 0 && <span className="absolute -top-1.5 -right-1.5 bg-primary text-white text-[0.6rem] w-4 h-4 rounded-full flex items-center justify-center font-bold">{basketCount}</span>}
        </div>
      </div>
    </header>
  );
